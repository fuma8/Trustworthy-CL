...
data_dir = '/home/kimishima/pytorch-classification-uncertainty/data/15-Scene'
...
def train(epoch, M, PhiR):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/scene15_original.jpg")
        for i in range(M):
            cnt += 1
            max_r = torch.max(cube_r[:,i,:,:])
            min_r = torch.min(cube_r[:,i,:,:])
            normarized_cube_r = (cube_r[:,i,:,:] - min_r) / (max_r - min_r)
            reconstructed_cube = normarized_cube_r.unsqueeze(1)
            ...
            # plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/scene15_cube_"+str(i)+".jpg")
            # exit()
            ...
            loss = ce_loss(targets_, outputs, num_class, epoch, 10, device) + torch.mul(criterion(phi_consR, I), gamma1)
    ...
    return total_accuracy, total_uncertainty, total_auc, PhiR

        

def test(epoch, M, PhiR, flag):
    global best_acc
    model.eval()
    test_loss = 0
    correct = 0
    ...
    if flag == True and acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        pth_path = './checkpoint/scene15_blk_size_'+str(blk_size)+'_b_'+str(batch_size)+'_learnable_matrix_proposed_loss.pth'
        dict_path = './checkpoint/scene15_blk_size_'+str(blk_size)+'_b_'+str(batch_size)+'_learnable_matrix_proposed_loss_dict.pth'
...
for epoch in range(total_epoch):
    train_acc, train_u, train_auc, trained_PhiR = train(epoch+1, M, PhiR)
    test_acc, test_u, test_auc = test(epoch+1, M, trained_PhiR, True)
    PhiR = trained_PhiR
...
torch.save(acc_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_scene15_acc_blk_"+str(blk_size)+"_b_"+str(batch_size)+".pkl")
uncertainty_dic = {}
uncertainty_dic["train"] = train_uncertainty_list
uncertainty_dic["test"] = test_uncertainty_list
torch.save(uncertainty_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_scene15_uncertainty_blk_"+str(blk_size)+"_b_"+str(batch_size)+".pkl")
auc_dic = {}
auc_dic["train"] = train_auc_list
auc_dic["test"] = test_auc_list
torch.save(auc_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_scene15_auc_blk_"+str(blk_size)+"_b_"+str(batch_size)+".pkl")
...
# 画像の保存と表示
plt.tight_layout()
plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/learnable_matrix_scene15_acc_uncertainty_proposed_loss_blk_"+str(blk_size)+"_b_"+str(batch_size)+".jpg")
plt.show()
...
cs_result_file = open("/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_scene15_result_blk_"+str(blk_size)+"_b_"+str(batch_size)+".txt", "w")
for i in range(1, M+1):
    print("SR=", i / M)
    acc_cs, u_cs, auc_cs = test(1, i, trained_PhiR, False)
...
# dataset = torchvision.datasets.ImageFolder(root="/home/kimishima/pytorch-classification-uncertainty/data/15-Scene", transform = transform)
...
# input_channel = 1
# # model = ResNet18(num_classes, input_channels = input_channel)
# model = ViT('B_16_imagenet1k', pretrained=False, in_channels = input_channel, image_size = cube_size, num_classes = num_classes)
# # summary(model=model, input_size=(batch_size*M, 1, cube_size, cube_size))
# # input()
...
# #     plt.subplot(131)
# #     plt.imshow(img, cmap="gray")
# #     plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/scene15.jpg")
# #     exit()
...
#         optimizer.zero_grad()
#         reconstructed_cube_all = torch.randn(0, 1, cube_size, cube_size).to(device)
...
#             min_r = torch.min(cube_r[:,i,:,:])
#             normarized_cube_r = (cube_r[:,i,:,:] - min_r) / (max_r - min_r)
#             reconstructed_cube = normarized_cube_r
#             reconstructed_cube = reconstructed_cube.unsqueeze(1)
...
#         if not os.path.isdir('checkpoint'):
#             os.mkdir('checkpoint')
#         pth_path = './checkpoint/scene15_blk_size_'+str(blk_size)+'_proposal.pth'
#         torch.save(model.state_dict(), pth_path)
#         best_acc = acc
#     print("Best Accuracy:", best_acc)
...
# uncertainty_dic = {}
# uncertainty_dic["train"] = train_uncertainty_list
# uncertainty_dic["test"] = test__uncertainty_list
# torch.save(uncertainty_dic, "/home/kimishima/pytorch-classification-uncertainty/scene15_uncertainty.pkl")
...
# # 画像の保存と表示
# plt.tight_layout()
# plt.savefig('/home/kimishima/pytorch-classification-uncertainty/images/scene15_acc_uncertainty_proposal.jpg')
# plt.show()
...
# cs_result_file = open("/home/kimishima/pytorch-classification-uncertainty/scene15_result_proposal.txt", "w")
# for i in range(1, M+1):
#     print("SR=", i / M)
