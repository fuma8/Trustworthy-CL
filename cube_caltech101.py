...
# summary(model=model, input_size=(batch_size, 3, cube_size, cube_size))
# input()
...
def test(epoch, M, PhiR, PhiG, PhiB, flag):
    ...
    if flag == True and acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        pth_path = './checkpoint/Caltech101_blk_size_'+str(blk_size)+'_learnable_matrix_proposed_loss.pth'
        dict_path = './checkpoint/Caltech101_blk_size_'+str(blk_size)+'_learnable_matrix_proposed_loss_dict.pth'
...
torch.save(acc_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_Caltech101_acc_blk_"+str(blk_size)+".pkl")
uncertainty_dic = {}
uncertainty_dic["train"] = train_uncertainty_list
uncertainty_dic["test"] = test_uncertainty_list
torch.save(uncertainty_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_Caltech101_uncertainty_blk_"+str(blk_size)+".pkl")
auc_dic = {}
auc_dic["train"] = train_auc_list
auc_dic["test"] = test_auc_list
torch.save(auc_dic, "/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_Caltech101_auc_blk_"+str(blk_size)+".pkl")
...
# 画像の保存と表示
plt.tight_layout()
plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/learnable_matrix_Caltech101_acc_uncertainty_proposed_loss_blk_"+str(blk_size)+".jpg")
plt.show()
...
cs_result_file = open("/home/kimishima/pytorch-classification-uncertainty/learnable_matrix_Caltech101_result_blk_"+str(blk_size)+".txt", "w")
...
#         y = torch.zeros(0, num_classes)
#         targets = torch.zeros(0).int()
#         PhiWeightR = PhiR.contiguous().view(M, 1, rows, cols).to(device)
#         PhiWeightG = PhiG.contiguous().view(M, 1, rows, cols).to(device)
#         PhiWeightB = PhiB.contiguous().view(M, 1, rows, cols).to(device)
#         cube_r = F.conv2d(inputs[:, 0:1, :, :], PhiWeightR, padding=0, stride=blk_size, bias=None)
...
#         phi_consG = torch.mm(PhiG, PhiG.t()).to(device)
#         phi_consB = torch.mm(PhiB, PhiB.t()).to(device)
#         loss = ce_loss(targets, outputs, num_class, epoch, 10, device) + torch.mul(criterion(phi_consR, I), gamma1) + torch.mul(criterion(phi_consG, I), gamma1) + torch.mul(criterion(phi_consB, I), gamma1)
...
#         loss.backward()
#         optimizer.step()
#         auc_score += multiclass_auroc(input = outputs, target = targets, num_classes = num_class, average = "macro").item() * 100
#         train_loss += loss.item()
#         _, predicted = outputs.max(1)
...
#     total_auc = auc_score / len(train_loader)
#     return total_accuracy, total_uncertainty, total_auc, PhiR, PhiG, PhiB
...
#     acc = 100.*correct/total
#     total_acc = correct/total
#     total_auc = auc_score/len(val_loader)
...
#         if not os.path.isdir('checkpoint'):
#             os.mkdir('checkpoint')
#         pth_path = './checkpoint/caltech101_blk_size_'+str(blk_size)+'_proposed_loss.pth'
#         dict_path = './checkpoint/scene15_blk_size_'+str(blk_size)+'_learnable_matrix_proposed_loss_dict.pth'
#         torch.save(model.state_dict(), pth_path)
#         torch.save(state, dict_path)
#         best_acc = acc
#     print("Best Accuracy:", best_acc)
...
# acc_dic["train"] = train_acc_list
# acc_dic["test"] = test_acc_list
# torch.save(acc_dic, "/home/kimishima/pytorch-classification-uncertainty/caltech101_acc_blk_"+str(blk_size)+".pkl")
# uncertainty_dic = {}
# uncertainty_dic["train"] = train_uncertainty_list
# uncertainty_dic["test"] = test_uncertainty_list
# torch.save(uncertainty_dic, "/home/kimishima/pytorch-classification-uncertainty/caltech101_uncertainty_blk_"+str(blk_size)+".pkl")
...
# # 画像の保存と表示
# plt.tight_layout()
# plt.savefig("/home/kimishima/pytorch-classification-uncertainty/images/caltech101_acc_uncertainty_proposed_loss_blk_"+str(blk_size)+".jpg")
# plt.show()
...
# cs_result_file = open("/home/kimishima/pytorch-classification-uncertainty/caltech101_result_blk_"+str(blk_size)+".txt", "w")
# for i in range(1, M+1):
#     print("SR=", i / M)
